...
bool TitleScreen::DrawScreen(const bool SkipTitle) {
	...
	while(!Input::ExitOperation()) {
		...
		if(Btn.DrawRadioButton(600, 500, "四角形", !StageTypeTmp.first && StageTypeTmp.second == StageForm::SQR)) {
			StageTypeTmp = std::make_pair(false, StageForm::SQR);
		}
		if(Btn.DrawRadioButton(700, 500, "六角形", !StageTypeTmp.first && StageTypeTmp.second == StageForm::HEX)) {
			StageTypeTmp = std::make_pair(false, StageForm::HEX);
		}
		// ステージの大きさの設定ＵＩ
		...
		// シード文字列の数値化
		if(SeedStr.empty() || SeedStr == "-") StageSeedTmp.second = 0; // 文字列が空なら 0 にする
		else try {
